...
shell_state_t shell_state;  // Shell state
...
int execute_command(char* command, char** args)
{
    ...
    if (strcmp(command, "") == 0) return 0;  // Empty command
...
{
    if (args[0] == NULL) { print_newline(); return 0; }

    print_newline();
    int i = 0;
    while (args[i] != NULL) {
        print_formatted_string(args[i], WHITE_COLOR);
        if (args[i + 1] != NULL) print_formatted_string(" ", WHITE_COLOR);
        i++;
    }
    return 0;
}
...
{
    if (args[0] == NULL) { 
        print_formatted_string("Usage: mkdir <directory_name>", RED); 
        print_newline(); 
        return -1; 
    }
...
{
    if (args[0] == NULL) { 
        print_formatted_string("Usage: cd <directory_name>", RED); 
        print_newline(); 
        return -1; 
    }
    ...
    // Handle relative, absolute, and special cases
    if (strcmp(args[0], "..") == 0) {
        if (strcmp(shell_state.current_path, "/") == 0) return 0;  // Already at root
    ...
    if (fs_change_directory(new_path)) {
        ...
        fs_list_directory(shell_state.current_path, &shell_state.current_directory);  // Update current directory
        return 0;
    } else {
